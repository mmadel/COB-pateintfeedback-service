...
public class PerformanceFeedbackService {
    ...
    public double retrieveHappyIndex(long dateFrom, long dateTo, long clinicId) {
        int[] feedbackValues = FeedbackCounter.count(dateFrom, dateTo, clinicId,serviceName);
        return HappyIndexFormula.calculate(100, feedbackValues[0], feedbackValues[1], feedbackValues[1]);
    ...
    public long retrieveNPS(long dateFrom, long dateTo, long clinicId) {
        ...
        return NPSFormula.calculate(total, feedbackValues[0], feedbackValues[2]);
    }


    public double retrieveAverage(long dateFrom, long dateTo, long clinicId) {
        int[] feedbackValues = FeedbackCounter.count(dateFrom, dateTo, clinicId,serviceName);
        return AverageFormula.calculate(feedbackValues[0], feedbackValues[1], feedbackValues[2]);
    }
}
